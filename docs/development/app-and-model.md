...
首页采用手记版布局：桌面左侧显示当前任务、概览、五维质量、三表处置、时间划分和产物；右侧为实验助手。手机上下排列，并提供结果与助手的快捷跳转。概览与可视化均读取当前任务，不预置演示数字；任务失败、未发布或新建会话时清空旧数据。模型栏只说明配置状态。

页面是包内 HTML / CSS / JavaScript，无构建步骤或外部字体依赖。浏览器刷新即可读取静态资源更新；`.env` 配置变更仍需重启 API。操作与验证记录见[界面说明](../iterations/01-governance/demo/界面说明.md)。
...
当前实现与验收见[解析与重试计划](../iterations/01-governance/plans/追问解析与重试完善.md)和[最新实测](../iterations/01-governance/reports/2026-09-25-追问解析与重试实测.md)：修正后原题 7/7、14 项变体与多轮检查首测 13/14 通过，包含两项应用澄清；一次 504 单独补测通过。真实页面新追问和旧失败重试均通过，原问题、原任务、重复请求缓存与失败保留得到验证。
