...
这个约束覆盖已支持的治理报告解释，**不是通用的自然语言事实验证器**：要点是否切题、是否遗漏用户关心的问题，仍取决于模型和审阅。[备用服务实测](../iterations/01-governance/reports/2026-09-25-备用模型全链路实测.md)中曾出现已读样例却只输出报告概述，以及“简短回答”仍选择 12 个要点；HTTP 200、`evidence_rendered` 和来源校验通过不能代替相关性检查。普通新任务、未完成任务的状态答复和新增的其他业务工具保留原有工具循环；后续算法结果应另设计对应的证据解释协议。
