...
**状态：架构草案，尚未实现。** 本文根据三轮项目需求和小组补充目标提出共同架构，作为迭代一实现及后续交接的依据；公共协议和首版选型已细化为建议方案，具体依赖组合仍待安装验证，模型服务仍待确定。
...
本地 Python 开发与运行使用 `duscwalk` 用户的 Conda 环境 `AgentDev`（`/home/duscwalk/miniconda3/envs/AgentDev`）。当前环境尚无 Python 和已安装的 Conda 软件包；首次运行项目代码前按需安装，并记录 Python 与依赖版本。环境选择已确定，具体安装与框架选型按实现需要推进。
...
每次工具调用记录会话标识、调用标识、工具版本、实际输入与输出；长任务调用另关联任务标识。查询无需制造一个长任务，但同样保留调用证据。统一返回结构区分 `completed`（即时查询完成）、`accepted`（长任务已受理）、`rejected`（前置校验未通过）和 `failed`（即时工具执行失败），分别承载结果、任务引用或错误；长任务执行失败由任务状态表达。
...
正常任务状态为 `queued → running → succeeded/failed`，当前阶段单独记录。中断后无法核实外部执行状态时使用 `unknown`（状态待核查），取得真实证据后再恢复为运行、成功或失败；不得直接重新排队。完整迁移、受理去重和发布规则见技术方案。
...
下面是小组根据可扩展目标新增的验收建议，代码实现后须实际验证。
