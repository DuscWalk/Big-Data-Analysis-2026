...
**状态：工具、任务、独立 worker、版本化产物和 Hadoop 治理已实现；模型、HTTP 会话与前端待接入。** 本文维护三轮共同架构；模块状态以[开发计划](../iterations/01-governance/开发与交接计划.md)为准。Python 和 Hadoop/JDK 组合已通过真实作业验证，模型服务仍待确定。
